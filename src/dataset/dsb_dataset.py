...
class DsbDataset(Dataset):
    ...
    def __init__(self, root: str, train: bool, transforms=None):
        super(DsbDataset, self).__init__()
        # 拼接路径
        ...
        # 获取图片名[列表]
        img_names = [i for i in os.listdir(os.path.join(data_root, "img")) if i.endswith(".png")]
        # 获取原图片列表
        self.img_list = [os.path.join(data_root, "img", i) for i in img_names]
        # 获取手动分割图片列表
        self.manual = [os.path.join(data_root, "gt_img", i.split(".")[0] + ".png")
                       for i in img_names]
        # check files
