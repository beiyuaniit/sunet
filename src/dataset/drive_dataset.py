...
class DriveDataset(Dataset):
    ...
    def __init__(self, root: str, train: bool, transforms=None):
        ...
        # 获取图片名[列表]
        img_names = [i for i in os.listdir(os.path.join(data_root, "img")) if i.endswith(".tif")]
        # 获取原图片列表
        self.img_list = [os.path.join(data_root, "img", i) for i in img_names]
        # 获取手动分割图片列表
        self.manual = [os.path.join(data_root, "gt_img", i.split("_")[0] + "_manual1.gif")
                       for i in img_names]
        # check files
    ...
    def collate_fn(batch):
        ...
        # todo 这个填充的是哪一部分？tian
        batched_imgs = cat_list(images, fill_value=0)
        batched_targets = cat_list(targets, fill_value=255) # 255
